...
const task_10_Result_1: HTMLElement | null = document.getElementById("task-1-result");
...
const task_10_Result_2: HTMLElement | null = document.getElementById("task-2-result");

function calculateSumOfArray(initialArray: any[]): string {
    let sum: number = 0;
    let resultText: string = initialArray
        .map((item: any) =>
            typeof item === "string"
                ? `"${item}"` : item === null
                    ? "null" : item === undefined
                        ? "undefined" : item)
        .join(", ");
    sum = initialArray.reduce((acc: number, element: any) => {
...
const task_10_Result_3: HTMLElement | null = document.getElementById("task-3-result");
...
const task_10_Result_4: HTMLElement | null = document.getElementById("task-4-result");
